...
public class CareerServiceImplementation implements CareerService {
	...
	public ApiResponse findAll(Pageable pageable) throws EmptyListException {
		List<Career> careers = careerRepository.findAll(pageable).getContent();
		if(careers.isEmpty())
	...
	public ApiResponse findByStudent(Integer id, Pageable pageable) throws ObjNotFoundException, EmptyListException {
		...
		List<Career> careers = careerRepository.findCareerByStudent(student.get().getName(), student.get().getSurname(), pageable);
		if(careers.isEmpty())
			throw new EmptyListException(String.format(StringUtils.EMPTY_LIST, StringUtils.EXAM+ "s"));
		return new ApiResponse(HttpStatus.FOUND, mapper.mapAll(careers, CareerDto.class));
	}
	...
	public ApiResponse findByCourse(String name, Pageable pageable) throws ObjNotFoundException, EmptyListException {
		...
		List<Career> careers = careerRepository.findCareerByCourse(name, pageable);
		if(careers.isEmpty())
			throw new EmptyListException(String.format(StringUtils.EMPTY_LIST, StringUtils.EXAM + "s"));
		return new ApiResponse(HttpStatus.FOUND, mapper.mapAll(careers, CareerDto.class));
	}
	...
	public ApiResponse findByCourseAndEvaluation(String name, Integer evaluation, Pageable pageable) throws ObjNotFoundException {
		...
		if(course == null)
			throw new ObjNotFoundException(String.format(StringUtils.NOT_FOUND_NAME, name));
		List<Career> careers = careerRepository.findCareerByCourseAndEvaluation(name, evaluation, pageable);
		if(careers.isEmpty())
